...
│  ┌────────────────────────────────────────────────────┐ │
│  │  routes.py (上传接口)                              │ │
│  │  - POST /api/v1/pdf/extract (用户上传)             │ │
│  │  - GET  /api/v1/pdf/extract/{id}                  │ │
│  │  - GET  /api/v1/pdf/extract (列表)                │ │
│  └────────────────────────────────────────────────────┘ │
│  ┌────────────────────────────────────────────────────┐ │
│  │  pdf_routes.py (处理接口 - 新增)                   │ │
│  │  - POST /api/v1/pdf/process (提交批量处理)         │ │
│  │    接收 oss_key_list，返回 task_id 列表            │ │
│  │  - GET /api/v1/pdf/process/{task_id} (查询状态)   │ │
│  │    返回完整的提取结果和下载链接                    │ │
...
│  │  - submit_extraction() (上传流程)                  │ │
│  │  - submit_extraction_from_oss() (处理流程 - 新增) │ │
│  │  - process_pdf() (核心处理逻辑)                    │ │
│  └────────────────────────────────────────────────────┘ │
│  ┌────────────────────────────────────────────────────┐ │
│  │  queue_tasks.py (Huey 队列)                        │ │
│  │  - pdf_extract_process_task() (异步任务)           │ │
│  │  - 自动重试: 3 次，60 秒间隔                       │ │
│  │  - get_queue_status() (队列状态查询)               │ │
│  └────────────────────────────────────────────────────┘ │
└──────────────────────┬─────────────────────────────────┘
                       │
┌──────────────────────────────────────────────────────────┐
...
└──────────────────────┬─────────────────────────────────┘
    ...
    ↓                  ↓                  ↓
┌────────┐        ┌──────────┐      ┌──────────┐
│  OSS   │        │ Qwen VL  │      │PostgreSQL│
│ Storage│        │ (通义千问)│      │ Database │
└────────┘        └──────────┘      └──────────┘
                       ↑
                   Redis (Huey)
```
